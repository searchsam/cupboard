  ...
  >
    <template v-slot="{ mutate, loading, error, data }">
      <form class="p-4" action="" method="POST" @submit.prevent="mutate()">
        <div class="mb-4">
          <input class="h-12 border" type="email" v-model="username" name="username" placeholder="Email" />
        </div>
        <div class="mb-4">
          <input class="h-12 border" name="password" type="password" v-model="password" placeholder="Password" />
        </div>
