<template>
    <div id="order">
      <h1 class="font-thin text-6xl">
        Solicitudes {{ this.$route.params.id }}
      </h1>
      ...
      <div id="createRequestForm" class="w-full bg-white m-2" v-if="orderStatus">
        ...
        <form class="p-4" action="" method="POST" @submit.prevent="createRequest">
          <input class="mr-2 h-12 w-64 border-2 border-gray-400 placeholder-gray-400" v-model="description" type="text" placeholder="Peticion">
          <input class="mr-2 h-12 border-2 border-gray-400 placeholder-gray-400" v-model="quantity" type="text" placeholder="Cantidad">
          <button
        ...
        >
          <div class="item-list">
            <p class="p-5">
              ...
              <span class="p-5">{{ request.quantity }} {{ request.description }}</span>
            </p>
          </div>
        </li>
      </ul>
  ...
  methods: {
      createRequest() {
        this.$apollo.mutate({
            ...
            variables: {
                data: { description: this.description, quantity: this.quantity }
            },
